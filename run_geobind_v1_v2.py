#!/usr/bin/env python
"""
GeoBind v1 — v2 Parameter Variant
==================================
Same pipeline as GeoBind v1 (pipe_geobind_v1) but with v2-aligned
structural parameters matching ToMe v2 adjustments.

Changes from run_geobind_v1.py (paper baseline):
  P0  eot_replace_step          40  → 30
  P1  merge weights (alpha/beta) 1.1/1.2 → 1.0/1.0
...
Usage
-----
    python run_geobind_v1_v2.py
    python run_geobind_v1_v2.py --output_dir eval_results --subsets color texture
"""
...
def setup_logging(output_dir):
    ...
    log_path = os.path.join(output_dir, f"geobind_v1_v2_log_{ts}.txt")
    ...
    logger = logging.getLogger("geobind_v1_v2")
...
log = logging.getLogger("geobind_v1_v2")
...
# ─────────────────────────────────────────────────────────
#  Image generation  (v2 parameters)
# ─────────────────────────────────────────────────────────

def generate_images(args):
    device = "cuda" if torch.cuda.is_available() else "cpu"

    log.info("Loading GeoBind v1 pipeline (v2 params) …")
...
def parse_args():
    p = argparse.ArgumentParser(description="GeoBind v1 (v2 params): generate images")
...
def main():
    ...
    log.info("═══  Generating GeoBind v1 (v2 params) images  ═══")
