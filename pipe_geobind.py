...
def contrastive_loss(
    ...
    for n_vec in other_anchors:
        sim_neg_list.append(F.cosine_similarity(query_, n_vec, dim=-1))  
    sim_neg = torch.cat(sim_neg_list, dim=0)  # (N_neg,)

    numerator = torch.exp(sim_pos / temp)  # shape=(1,)
    denominator = numerator + torch.exp(sim_neg / temp).sum()
...
def contrastive_loss_hyperspace(
    ...
    """
    Contrastive loss in hyperbolic space with batch and sequence length handling.
    """
    device = query_vec.device
...
class SinusoidalPositionalEncoding(nn.Module):
    def __init__(self, embed_dim: int, max_length: int = 128):
        ...
        position = torch.arange(0, max_length).unsqueeze(1)  # (max_length, 1)
