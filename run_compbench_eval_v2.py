#!/usr/bin/env python
"""
T2I-CompBench Evaluation — v2 paper-aligned + shape tuning
===========================================================
Based on run_compbench_eval_paper.py with 5 further adjustments:

  P0. eot_replace_step    40 → 30    (earlier ETS for stronger EOT cleanup)
  P1. token merge weights 1.1/1.2 → 1.0/1.0  (paper: simple addition)
  P2. attention_refinement_steps [4,4] → [5,4]  (more early-step iterations)
  P3. thresholds steps 7-9: 21 → 20.5  (slightly less early stopping)
  P4. token_refinement_steps 3 → 4  (more semantic binding iterations)

Usage
-----
    python run_compbench_eval_v2.py --methods ToMe --phase generate
    python run_compbench_eval_v2.py --methods SDXL ToMe --phase all
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="T2I-CompBench eval v2 (paper-aligned + shape tuning)")
    ...
    p.add_argument("--output_dir", default="eval_results_v2")
...
def _build_call_kwargs(method, prompt, args, ti, pa, merged, pl, controller,
    ...
    base = dict(
        ...
        prompt3=merged,
        prompt_length=pl,
        token_refinement_steps=4,          # P4: 3 → 4
        attention_refinement_steps=[5, 4], # P2: [4,4] → [5,4]
        tome_control_steps=[10, 10],
        eot_replace_step=30,               # P0: 40 → 30
        use_pose_loss=False,
        negative_prompt="low res, ugly, blurry, artifact, unreal",
        merge_noun_weight=1.0,             # P1: 1.1 → 1.0 (paper: simple addition)
        merge_attr_weight=1.0,             # P1: 1.2 → 1.0 (paper: simple addition)
    )
...
def generate_all_images(args):
    ...
    thresholds = {                             # P3: steps 7-9 slightly lowered
...
def print_result_table(results):
    ...
    log.info("")
    log.info("=" * 62)
    log.info("  T2I-CompBench  BLIP-VQA Scores  (v2)")
    log.info("=" * 62)
    header = f"  {'Method':<16}"
    for s in subsets:
...
def main():
    ...
    log.info("=== v2 evaluation: paper-aligned + shape tuning ===")
    log.info("  P0 eot_replace_step=30  P1 merge_weights=1.0/1.0")
    log.info("  P2 attn_refine=[5,4]  P3 thresholds[7-9]=20.5  P4 token_refine=4")
