...
\State Add positional encoding via M\"obius addition (Eq.~5)
\For{each entity $k = 1, \ldots, K$}
    \State Euclidean merge: $\hat{c}_k^{\text{E}} \leftarrow \alpha \cdot c_{n_k} + \beta \cdot \sum_{j} c_{a_{k,j}}$ \Comment{Eq.~12}
    \State Hyperbolic merge: $\hat{c}_k^{\mathcal{H}} \leftarrow$ attention-based fusion in $\mathcal{H}$ \Comment{Eq.~13--14}
...
\For{$t = T, T-1, \ldots, 1$}
    \If{$t > T - T_{\text{tok}}$} \Comment{Token optimization phase}
        \For{iter $= 1, \ldots, 3$}
            \State Compute $\mathcal{L}_{\text{sem}}$ from cross-attention maps \Comment{Eq.~16--17}
            \State Compute $\mathcal{L}_{\text{hyp}}$ from geodesic distances \Comment{Eq.~9}
...
\paragraph{Hardware.} Experiments are conducted on NVIDIA H200 (141 GB) GPUs. Single-image generation takes 12.8--16.4 seconds depending on entity count (Table~7 in the main paper).
...
\item \textbf{Memory}: Peak GPU memory usage is $\sim$18\,GB (SDXL) on H200, dominated by the base model weights. GeoBind's geometric operations add negligible memory overhead ($<$100\,MB).
\end{itemize}
...
\item \texttt{run\_compbench\_eval.py}: Full T2I-CompBench evaluation pipeline (generation + BLIP-VQA scoring) for all methods.
...
Evaluation on T2I-CompBench uses the official evaluation code from~\cite{huang2023t2icompbench}. GOB-Bench evaluation uses GPT-4o and Qwen2-VL APIs as described in Sec.~4.1.
