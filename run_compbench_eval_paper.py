#!/usr/bin/env python
"""
T2I-CompBench Evaluation — Paper-aligned ToMe hyperparameters
=============================================================
Based on run_compbench_eval.py with two parameter changes to match the
NeurIPS 2024 paper (2411.07132v1):

  1. eot_replace_step  60 → 40   (enable ETS within 50 inference steps)
  2. tome_control_steps [5,5] → [10,10]  (Topt = 20% of T = 10 steps)

Attention extraction already uses decoder-only layers (up_blocks.0
transformer_blocks 1-3) via the greenlist in register_attention_control,
matching the paper's "first three layers of the decoder".

Usage
-----
    python run_compbench_eval_paper.py --methods ToMe --phase generate
    python run_compbench_eval_paper.py --methods SDXL ToMe --phase all
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="T2I-CompBench eval (paper-aligned ToMe hyperparams)")
    ...
    p.add_argument("--output_dir", default="eval_results_paper")
...
def _build_call_kwargs(method, prompt, args, ti, pa, merged, pl, controller,
    ...
    base = dict(
        ...
        tome_control_steps=[10, 10],       # paper: Topt = 0.2T = 10 steps
        eot_replace_step=40,               # paper: enable ETS (< n_inference_steps)
        use_pose_loss=False,
...
def print_result_table(results):
    ...
    log.info("")
    log.info("=" * 62)
    log.info("  T2I-CompBench  BLIP-VQA Scores  (paper-aligned)")
    log.info("=" * 62)
    header = f"  {'Method':<16}"
    for s in subsets:
...
def main():
    ...
    log.info("=== Paper-aligned ToMe evaluation ===")
    log.info("  Changed vs original: tome_control_steps=[10,10], eot_replace_step=40")
