#!/usr/bin/env python
"""
GeoBind (Improved) — v2 Parameter Variant
==========================================
Improved GeoBind pipeline (pipe_geobind.py) with v2-aligned structural
parameters and algorithm fixes:

  Algorithm fixes (in pipe_geobind.py):
    - Euclidean cosine contrastive loss (replaces unstable hyperbolic)
    - NaN/inf gradient guard (replaces aggressive norm clipping)
    - Restored step_size=10000, lambda_reg=0.1
    - Merge weights alpha/beta default to 1.0/1.0

  v2 structural parameter changes:
    P0  eot_replace_step           40  → 30
    P1  merge weights              1.1/1.2 → 1.0/1.0  (now default)
    P2  attention_refinement_steps [4, 4]  → [5, 4]
    P3  thresholds steps 7-9      21      → 20.5
    P4  token_refinement_steps      3     → 4

  GeoBind-specific tuning:
    - tome_control_steps [10, 12]: more aggressive since contrastive
      loss is now stable and provides real binding separation
    - hyper_weight=0.15: controlled hyperbolic geometry contribution

Usage
-----
    python run_geobind_v2.py
    python run_geobind_v2.py --output_dir eval_results --subsets color texture
"""
...
def setup_logging(output_dir):
    ...
    log_path = os.path.join(output_dir, f"geobind_v2_log_{ts}.txt")
    ...
    logger = logging.getLogger("geobind_v2")
...
log = logging.getLogger("geobind_v2")
...
# ─────────────────────────────────────────────────────────
#  Image generation  (v2 params + algorithm improvements)
# ─────────────────────────────────────────────────────────

def generate_images(args):
    device = "cuda" if torch.cuda.is_available() else "cpu"

    log.info("Loading GeoBind (improved, v2) pipeline …")
...
def parse_args():
    p = argparse.ArgumentParser(description="GeoBind v2 (improved): generate images")
...
def main():
    ...
    log.info("═══  Generating GeoBind v2 (improved) images  ═══")
